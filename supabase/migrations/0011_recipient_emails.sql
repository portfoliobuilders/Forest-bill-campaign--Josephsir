-- Multiple TO recipients, Forest Bill routing, and the 12 canonical clauses.
-- Does not set is_active = true.
...
where cardinality(recipient_emails) = 0
...
set
  ...
  summary_ml = '2024 നവംബർ 1-ലെ ഗസറ്റ് ബിൽ 228. പൊതുജന അഭിപ്രായത്തിന് 2024 ഡിസംബർ 31 വരെ സമയമുണ്ടായിരുന്നു. ആ കൂടിയാലോചന അവസാനിച്ചു; കാബിനറ്റ് ബിൽ പിൻവലിച്ചു. പുതിയ ഔദ്യോഗിക കൂടിയാലോചന വരുന്നതുവരെ ഇത് സജീവമല്ല.',
  summary_en = 'Gazette Bill 228 of 1 November 2024. Public comments closed on 31 December 2024. The Cabinet later dropped the Bill. This campaign stays inactive until a new official consultation is verified.',
  ...
  intro_ml = 'കേരള ഫോറസ്റ്റ് നിയമ ഭേദഗതി 2024-നോട് ഞാൻ താഴെപ്പറയുന്ന ആശങ്കകളിൽ എതിർപ്പ് രേഖപ്പെടുത്തുന്നു.',
  intro_en = 'I record my objection to the Kerala Forest (Amendment) Bill, 2024, on the points below.',
  ...
  source_url = 'https://prsindia.org/files/bills_acts/bills_states/kerala/2024/Bills228of2024KL.pdf'
where slug in ('demo', 'kerala-forest-amendment-2024');

update public.objection_clauses oc
set is_active = false
from public.campaigns c
where oc.campaign_id = c.id
  and c.slug = 'kerala-forest-amendment-2024'
  and oc.code not in (
    'C1_OFFICER','C2_RIVER','C3_SEARCH','C4_VEHICLE','C5_ARREST','C6_OBSTRUCT',
    'C7_LOCKUP','C8_BNSS','C9_PRESUME','C10_CERTIFY','C11_PENALTY','C12_WILDLIFE'
  );

insert into public.objection_clauses (
  campaign_id, code, section_ref, title_ml, title_en,
  explain_ml, explain_en, email_ml, email_en, sort_order, is_active
)
select
  c.id,
  v.code,
