...
declare
  ...
  v_exists boolean;
begin
  select exists(select 1 from public.campaigns where slug = 'esa-draft-notification') into v_exists;
...
Select one of the concerns below and use the platform to prepare an email to the relevant authorities.$en$,
      ...
      'പരിസ്ഥിതിലോല പ്രദേശം (ESA) കരട് വിജ്ഞാപനം — അഭിപ്രായം',
      'Representation regarding the Ecologically Sensitive Area (ESA) Draft Notification',
...
{{full_name}}$ent$,
  ...
  if not exists (select 1 from public.campaign_recipients where campaign_id = v_id) then
  ...
  if not exists (select 1 from public.campaign_form_fields where campaign_id = v_id) then
    insert into public.campaign_form_fields (campaign_id, field_key, label_en, label_ml, is_enabled, is_required, display_order) values
      ...
      (v_id, 'email',           'Email',                     'ഇമെയിൽ',                             true,  true,  2),
      (v_id, 'phone',           'Mobile number',             'മൊബൈൽ നമ്പർ',                         true,  false, 3),
      (v_id, 'district',        'District',                  'ജില്ല',                               true,  false, 4),
      (v_id, 'local_body',      'Panchayat / Municipality',  'പഞ്ചായത്ത് / മുനിസിപ്പാലിറ്റി',       true,  false, 5),
      (v_id, 'village',         'Village',                   'വില്ലേജ്',                            true,  false, 6),
      (v_id, 'address',         'Address',                   'വിലാസം',                              true,  false, 7),
      (v_id, 'custom_message',  'Additional message',        'അധിക സന്ദേശം',                        true,  false, 8);
  end if;

  if not exists (select 1 from public.objection_clauses where campaign_id = v_id) then
