...
export function Step3_Verify({
  ...
  if (!siteKey) {
    return (
      <div>
        <h1 className="font-display text-2xl text-ink sm:text-3xl">{t(lang, 'verify')}</h1>
  ...
  return (
    <div>
      <h1 className="font-display text-2xl text-ink sm:text-3xl">{t(lang, 'verify')}</h1>
      <p className="mt-2 text-base leading-relaxed text-body">{t(lang, 'verifyIntro')}</p>
      {mode === 'preview' ? (
        <p className="mt-2 text-sm leading-relaxed text-amber-900">{t(lang, 'verifyPreviewNote')}</p>
      ) : null}
      <p className="mt-1 text-sm text-muted">
        {t(lang, 'otpSentTo')}: <span className="font-medium text-ink">{details.email}</span>
      </p>
      ...
      {phase === 'turnstile' ? (
        <div className="mt-4">
          ...
          />
          {busy ? <p className="mt-2 text-sm text-muted">{t(lang, 'verifyWorking')}</p> : null}
        </div>
      ) : null}
      ...
      {phase === 'otp' || phase === 'verified' ? (
        <form onSubmit={(e) => void handleVerify(e)} className="mt-5">
          ...
          <input
            ...
            className={cx(inputClass, 'text-lg tracking-widest')}
          />

          <div className="mt-3 flex flex-wrap items-center gap-2 text-sm">
            {resendSec > 0 ? (
              <span className="text-muted">{t(lang, 'resendCooldown').replace('{sec}', String(resendSec))}</span>
            ) : (
              <button
                type="button"
                disabled={busy || phase === 'verified'}
                onClick={() => void handleResend()}
                className={cx('min-h-11 font-semibold text-accent underline', focusRing)}
              >
                {t(lang, 'resendOtp')}
              </button>
            )}
            {otpSent ? <span className="text-muted">{t(lang, 'otpSentHint')}</span> : null}
          </div>

          {phase === 'verified' ? (
            <p className="mt-3 text-base font-medium text-accent" aria-live="polite">
              {t(lang, 'verifySuccess')}
            </p>
          ) : (
            <button type="submit" disabled={otp.length !== 6 || busy} className={cx(btnPrimary, 'mt-5')}>
              {t(lang, 'verifySubmit')}
            </button>
          )}
        </form>
      ...
      {phase === 'verified' ? (
        <button type="button" onClick={onContinue} className={cx(btnPrimary, 'mt-5 w-full sm:w-auto')}>
          {t(lang, 'continueToLetter')}
        </button>
      ) : null}

      {errorKey ? (
        <p className="mt-4 rounded-[8px] border border-red-300 bg-red-50 px-3 py-2 text-sm text-red-900" role="alert">
